...
check_grep() {
  ...
  if grep -qF "$pattern" "$file" 2>/dev/null; then green "$name"; PASS=$((PASS + 1))
...
test_npm() {
  ...
  check_grep "root ui script"            'npm -w @repo/ui run ui:add'   test-npm/package.json
  check_grep "shadcn-add uses npx"       'npx ${ver}'         test-npm/packages/ui/scripts/shadcn-add.mjs
...
test_pnpm() {
  ...
  check_grep "root ui script"             'pnpm --filter @repo/ui ui:add' test-pnpm/package.json
  check_grep "shadcn-add uses pnpm dlx"   'pnpm dlx ${ver}'    test-pnpm/packages/ui/scripts/shadcn-add.mjs
...
test_yarn() {
  ...
  check_grep "root ui script"             'yarn workspace @repo/ui ui:add' test-yarn/package.json
  check_grep "shadcn-add uses npx"        'npx ${ver}'          test-yarn/packages/ui/scripts/shadcn-add.mjs
...
test_bun() {
  ...
  check_grep "root ui script"             'bun --filter @repo/ui ui:add'  test-bun/package.json
  check_grep "shadcn-add uses bunx --bun" 'bunx --bun ${ver}'  test-bun/packages/ui/scripts/shadcn-add.mjs
...
test_auto() {
  ...
  check_grep "pnpm detect"  'pnpm --filter @repo/ui ui:add'  test-auto-pnpm/package.json
  ...
  check_grep "yarn detect"  'yarn workspace @repo/ui ui:add'  test-auto-yarn/package.json

  rm -rf test-auto-bun
  env npm_config_user_agent="bun/1.2.21 npm/? node/v24.7.0 linux x64" \
    $BIN test-auto-bun --no-install > /dev/null 2>&1
  check_grep "bun detect"   'bun --filter @repo/ui ui:add'   test-auto-bun/package.json

  rm -rf test-auto-npm
  env npm_config_user_agent="npm/11.12.1 node/v24.7.0 linux x64" \
    $BIN test-auto-npm --no-install > /dev/null 2>&1
  check_grep "npm detect"   'npm -w @repo/ui run ui:add'     test-auto-npm/package.json
}
